        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):  # 取batch
                ...
                dec_inp = torch.cat([batch_y[:, :self.args.label_len, :], dec_inp], dim=1).float().to(
                    self.device)  # 拼接 label_len长度 和 pred_len 的 0 序列
                # encoder - decoder
                if self.args.use_amp:  # 混合精度训练
                    with torch.cuda.amp.autocast():  # 自动混合精度
                        outputs = self.model(batch_x, batch_x_mark, dec_inp,
                                             batch_y_mark)  # 进入模型的输入是 batch_x, batch_x_mark, dec_inp, batch_y_mark 分别是 x, x的时间编码, 解码器输入, y的时间编码
                else:
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                pred = outputs  # 预测值
                true = batch_y  # 真实值
                true_save = np.concatenate((batch_x.cpu(), batch_y), axis=1)  # 为了结果多前半部分

                preds.append(pred)
                trues.append(true)
                trues_saves.append(true_save)
                # 每 20 次保存一次结果并可视化
                if i % 20 == 0:
                    input = batch_x.detach().cpu().numpy()
                    if test_data.scale and self.args.inverse:
                        shape = input.shape
                        input = test_data.inverse_transform(input.reshape(shape[0] * shape[1], -1)).reshape(
                            shape)  # 反标准化 + reshape
                    gt = np.concatenate((input[0, :, -1], true[0, :, -1]), axis=0)  # 真实值 ground truth
                    pd = np.concatenate((input[0, :, -1], pred[0, :, -1]), axis=0)  # 预测值 prediction
                    visual(gt, pd, os.path.join(folder_path, str(i) + '.svg'))  # 可视化

            preds = np.concatenate(preds, axis=0)
            trues = np.concatenate(trues, axis=0)
            trues_saves = np.concatenate(trues_saves, axis=0)
            # 两次reshape 为了计算dtw
        ...
        np.save(folder_path + 'true_save.npy', trues_saves)
        return
