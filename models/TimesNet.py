...
class TimesBlock(nn.Module):
    ...
    def forward(self, x):
        B, T, N = x.size()
        period_list, period_weight = FFT_for_Period(x, self.k)  # 得到周期和权重

        res = []
        for i in range(self.k): # 遍历topK的频率
            period = period_list[i] # 取这些topK对应的周期
            # padding
            if (self.seq_len + self.pred_len) % period != 0:    # 如果不是周期的整数倍 就做padding
                length = (
                                 ((self.seq_len + self.pred_len) // period) + 1) * period
                padding = torch.zeros([x.shape[0], (length - (self.seq_len + self.pred_len)), x.shape[2]]).to(x.device)
                out = torch.cat([x, padding], dim=1)    # 把数据和padding concat起来
            else:
                length = (self.seq_len + self.pred_len)
                out = x
            # reshape
            out = out.reshape(B, length // period, period,
                              N).permute(0, 3, 1, 2).contiguous()       # 按照周期进行reshape (B, N, T//P, P)
